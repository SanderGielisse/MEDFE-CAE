...
# SE MODEL - Squeezeand-Excitation (SE) block
...
class ConvDown(nn.Module):
    def __init__(self, in_c, out_c, kernel, stride, padding=0, dilation=1, groups=1, bias=False, layers=1,
                        activ=nn.LeakyReLU(0.2, False)):
                ...
                if activ is None and layers == 1:
                    ...
                    sequence += [
                        nn.Conv2d(nf_mult_prev * in_c, nf_mult * in_c,
                                  kernel_size=kernel, stride=stride, padding=padding, bias=bias),
                        nn.InstanceNorm2d(nf_mult * in_c),
                        activ if activ is not None else nn.LeakyReLU(0.2, False)
                    ]
...
class PCconv(nn.Module):
    def __init__(self, original):
        ...
        self.down_16 = ConvDown(512, 512, 4, 2, padding=1, activ=None)
        self.down_8 = ConvDown(512, 512, 4, 2, padding=1, layers=2, activ=None)
        self.down_4 = ConvDown(512, 512, 4, 2, padding=1, layers=3, activ=None)


        self.down = ConvDown(768, 256, 1, 1)
        if not self.original:
            # our proposed variant needs tanh activations at the encodings
            self.down_tanh = ConvDown(768, 256, 1, 1, activ=nn.Tanh())
    ...
    def forward(self, input, mask):
        ...
        # original has relu, proposed needs tanh
        final_down = self.down if self.original else self.down_tanh
