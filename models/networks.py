...
def define_G(input_nc, output_nc, ngf,  norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], init_gain=0.02, device=None, original=True):
    ...
    PCBlock = PCblock(stde_list, original=original, device=device, gpu_ids=gpu_ids)
...
class PCblock(nn.Module):
    def __init__(self, stde_list, original=True, device=None, gpu_ids=[]):
        super(PCblock, self).__init__()
        self.pc_block = PCconv(original=original)
        innerloss = InnerCos(original=original, device=device, gpu_ids=gpu_ids)
