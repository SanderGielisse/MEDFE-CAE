...
class MEDFE(BaseModel):
    def __init__(self, opt):
        ...
        self.original = not self.opt.high_dim_l1
    ...
    def forward(self):
        combined = torch.cat([self.input_DE, self.inv_ex_mask], 1)
        fake_p_1, fake_p_2, fake_p_3, fake_p_4, fake_p_5, fake_p_6 = self.netEN(combined)
    ...
    def backward_G(self):
        ...
        for loss in self.stde_loss:
            self.stde_loss_value += loss.get_loss()

        if self.original:
            self.stde_loss_value *= 4.0
        else:
            self.stde_loss_value *= 32.0 # otherwise this loss is way too small
        # print('stde_loss_value', self.stde_loss_value)
        # print('loss_G', self.loss_G)

        # multiply by 4, because old implementation called backwards() twice and also summed the loss twice?
